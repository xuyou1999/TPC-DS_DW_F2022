...
alter table web_returns add constraint wr_ret_cd2 foreign key  (wr_returning_cdemo_sk) references customer_demographics (cd_demo_sk);
alter table web_returns add constraint wr_ret_c2 foreign key  (wr_returning_customer_sk) references customer (c_customer_sk);
alter table web_returns add constraint wr_ret_cd3 foreign key  (wr_returning_hdemo_sk) references household_demographics (hd_demo_sk);
